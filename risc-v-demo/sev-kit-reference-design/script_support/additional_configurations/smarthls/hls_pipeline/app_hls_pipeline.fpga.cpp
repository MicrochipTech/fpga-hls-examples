...
int main(int argc, char** argv) {
    ...
    void* hls_pipeline_virt_addr = hls_pipeline_setup(HLS_PIPELINE_BASE_ADDR);
    if (hls_pipeline_virt_addr == NULL) {
        printf("Error: hls_pipeline_setup function failed\n"); fflush(stdout);
        exit(EXIT_FAILURE);
    }

    printf("Running watermark_setup function\n"); fflush(stdout);
    void* watermark_virt_addr = watermark_setup(WATERMARK_BASE_ADDR);
    if (watermark_virt_addr == NULL) {
        printf("Error: watermark_setup function failed\n"); fflush(stdout);
        exit(EXIT_FAILURE);
    }

    printf("Running rotozoom_setup function\n"); fflush(stdout);
    void* rotozoom_virt_addr = rotozoom_setup(ROTOZOOM_BASE_ADDR);
    if (rotozoom_virt_addr == NULL) {
        printf("Error: rotozoom_setup function failed\n"); fflush(stdout);
        exit(EXIT_FAILURE);
    }

    printf("Running get_frame_setup function\n"); fflush(stdout);
    void* get_frame_virt_addr = get_frame_setup(GET_FRAME_BASE_ADDR);
    if (get_frame_virt_addr == NULL) {
        printf("Error: get_frame_setup function failed\n"); fflush(stdout);
        exit(EXIT_FAILURE);
    }

    printf("Running put_frame_setup function\n"); fflush(stdout);
    void* put_frame_virt_addr = put_frame_setup(PUT_FRAME_BASE_ADDR);
    if (put_frame_virt_addr == NULL) {
