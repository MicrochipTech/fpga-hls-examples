...
void Conv(
    Tensor<INPUT_SIZE, INPUT_DEPTH> input,
    Tensor<OUTPUT_SIZE, OUTPUT_DEPTH> output,
    const short weights[OUTPUT_DEPTH][INPUT_DEPTH][FILTER_SIZE * FILTER_SIZE],
    ...
    for (unsigned r = 0; r < OUTPUT_SIZE; r++) {
        for (unsigned c = 0; c < OUTPUT_SIZE; c++) {
            for (unsigned od = 0; od < OUTPUT_DEPTH; od += PARALLEL_KERNELS) {
...
template <unsigned DEPTH, unsigned INPUT_SIZE, unsigned OUTPUT_SIZE,
          unsigned FILTER_SIZE, unsigned STRIDE, unsigned LAYER_INDEX>
...
// FC implements a generic fully-connected layer.
template <unsigned INPUT_DEPTH, unsigned INPUT_SIZE, unsigned OUTPUT_DEPTH,
          unsigned LAYER_INDEX>
void FC(Tensor<INPUT_SIZE, INPUT_DEPTH> input,
        DType output[OUTPUT_DEPTH]) {
