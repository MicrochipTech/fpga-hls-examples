...
## Generating the Bitstream and RISC-V Binaries Without a Board

The steps above assume an Icicle Kit is attached. If you do not have a
board, or you only need the build artifacts themselves (the FlashPro
Express job file and the RISC-V executables), every step except
programming and running can still be completed. Only two operations
actually require hardware:

<table>
<thead>
<tr class="header">
<th><strong>SoC flow step</strong></th>
<th><strong>Board required?</strong></th>
<th><strong>Reason</strong></th>
</tr>
</thead>
<tbody>
<tr class="odd">
<td>Generate Libero design</td>
<td>No</td>
<td>Creates the Libero/SmartDesign project only</td>
</tr>
<tr class="even">
<td>RTL synthesis</td>
<td>No</td>
<td>Runs in batch mode</td>
</tr>
<tr class="odd">
<td>Place-and-route and generate bitstream</td>
<td>No</td>
<td>Produces <code>Icicle_SoC.job</code></td>
</tr>
<tr class="even">
<td>Cross-compile software (with or without drivers)</td>
<td>No</td>
<td>Produces the <code>.elf</code> files</td>
</tr>
<tr class="odd">
<td>Program board</td>
<td><strong>Yes</strong></td>
<td>Requires a FlashPro programmer attached over JTAG</td>
</tr>
<tr class="even">
<td>Run software with / without accelerators</td>
<td><strong>Yes</strong></td>
<td>Copies the binary to the board over SSH using <code>BOARD_IP</code></td>
</tr>
</tbody>
</table>

<p align="center">Figure 7‑8 Board Requirement for Each SoC Flow Step</p>

`BOARD_IP` is read only by the two *Run software* steps. While generating
artifacts it may be left unset, left commented out, or set to a
placeholder value.

![](.//media/image2.png) **Do not use *Run software with accelerators* to
produce these files.** That option chains through *Program board*, which
fails with `Error: No programmers found` when no programmer is attached.
The failure aborts the flow *before* the job file is exported, leaving
`hls_output/soc/designer/MPFS_ICICLE_KIT_BASE_DESIGN/export/` empty and no
`.job` file anywhere in the project. Run the individual steps below
instead.

### Option 1: Using the SmartHLS IDE

![](.//media/image3.png) To generate the bitstream, select *SmartHLS -\>
RISC-V SoC Features (available for PolarFire SoC only) -\> Reference SoC
with HLS Accelerator(s) -\> Place-and-route and generate bitstream*.

SmartHLS first runs *Generate Libero design* and *RTL synthesis* if they
are out of date, then places and routes the design, verifies timing,
updates the Hart Software Services (HSS), and exports the FlashPro
Express job file. Libero is invoked in batch mode automatically, so there
is no need to open the Libero GUI, run *Generate FPGA Array Data*, or use
*Export FlashPro Express Job* by hand. Doing the export manually is not
equivalent: the manual dialog exports only the `FABRIC_SNVM` components
and skips the timing verification, the HSS/eNVM update, and the sNVM-to-SPI
relocation of design initialization data that the SmartHLS step performs.

![](.//media/image3.png) To generate the RISC-V executable that drives the
accelerators, select *SmartHLS -\> RISC-V SoC Features (available for
PolarFire SoC only) -\> Reference SoC with HLS Accelerator(s) -\>
Cross-compile software with accelerator drivers*.

![](.//media/image3.png) Optionally, to also generate the software-only
executable, select *SmartHLS -\> RISC-V SoC Features (available for
PolarFire SoC only) -\> Base SoC with no HLS Accelerators -\>
Cross-compile software for RISC-V*.

The hardware and software paths of the flow are independent (see Figure
6‑26), so the cross-compile steps do not require the bitstream to exist
and the three steps above may be run in any order.

### Option 2: Using the Command Line

Add `<SmartHLS Installation Path>/SmartHLS/bin` to your PATH environment
variable, change into the SmartHLS project directory, then run the
targets directly. On Linux:

```bash
export PATH=<SmartHLS Installation Path>/SmartHLS/bin:$PATH
cd <workspace>/vector_add_soc

shls -a soc_accel_proj_pnr           # bitstream  -> Icicle_SoC.job
shls -a soc_sw_compile_accel         # software   -> vector_add_soc.accel.elf
shls -a soc_sw_compile_no_accel      # optional   -> vector_add_soc.no_accel.elf
```
...
shls.bat -a soc_accel_proj_pnr
shls.bat -a soc_sw_compile_accel
shls.bat -a soc_sw_compile_no_accel
```

<p align="center">Figure 7‑9 Generating Artifacts from the Command Line</p>

These targets are the exact equivalents of the IDE menu items in Option
1. The `-a` option builds any out-of-date prerequisite steps without
prompting; use `-s` instead to skip out-of-date prerequisites and run only
the requested target. Note that `soc_accel_proj_pnr` covers synthesis,
place-and-route, timing verification, the HSS update, and the job export
in a single target, so no separate export command is needed.
...
<th><strong>Path (relative to the SmartHLS project directory)</strong></th>
</tr>
</thead>
<tbody>
<tr class="odd">
<td>FlashPro Express job</td>
<td><code>hls_output/soc/designer/MPFS_ICICLE_KIT_BASE_DESIGN/Icicle_SoC.job</code></td>
</tr>
<tr class="even">
<td>Executable with accelerator drivers</td>
<td><code>hls_output/&lt;project name&gt;.accel.elf</code></td>
</tr>
<tr class="odd">
<td>Software-only executable</td>
<td><code>hls_output/&lt;project name&gt;.no_accel.elf</code></td>
</tr>
<tr class="even">
<td>Generated Libero project</td>
<td><code>hls_output/soc/Icicle_SoC.prjx</code></td>
</tr>
</tbody>
</table>

<p align="center">Figure 7‑10 Generated Artifact Locations</p>

![](.//media/image2.png) Two things to be aware of when inspecting these
paths:

  - *Generate Libero design* deletes and recreates the entire
    `hls_output/soc` directory on every run. If you look inside it while
    generation is still in progress, `Icicle_SoC.prjx` will appear to be
    missing. Wait for the step to finish before concluding that anything
    failed.

  - A second, unrelated project file exists at
    `hls_output/synthesis/<project name>/<project name>.prjx`. That one
    belongs to the IP flow described in the [SmartHLS IP
    Flow](#smarthls-ip-flow) section, not to the SoC flow.

![](.//media/image2.png) Close the Libero GUI before running
`soc_accel_proj_pnr` or the equivalent IDE menu item. The step launches
its own Libero batch process against the same project, and an open GUI
session holding that project can cause the run to fail on locked files.
...
`icicle-kit-reference-design`. Open it, and you should see the following
...
![](.//media/image2.png) **If no Icicle Kit is attached, do not use
`compile_sw.shls.sh`.** The script runs under `set -eu` and its first
action is an `ssh root@$BOARD_IP` command to clean the board, so it aborts
before either compilation step runs when `BOARD_IP` is unset or the board
is unreachable. The compilation itself does not need the board at all —
invoke the two targets directly instead. On Linux:

```bash
cd script_support/additional_configurations/smarthls/invert_and_threshold
shls -a soc_sw_compile_accel         # -> hls_output/invert_and_threshold.accel.elf
shls -a soc_sw_compile_no_accel      # -> hls_output/invert_and_threshold.no_accel.elf
```
...
Note that only the `.sh` variants of `compile_sw.shls` and `run_sw.shls`
are shipped in the `invert_and_threshold` folder, so on Windows the
commands above are the practical way to build the executables regardless
of whether a board is present. Neither target reads `BOARD_IP`, and
neither involves Libero.

Because these targets do not clean `hls_output` first, take care when
switching the `SRCS` variation: a stale `.elf` from a previous variation
will remain alongside the newly built one. Run `shls clean` (or rebuild
both targets) after changing `SRCS` so that the two executables always
come from the same source variation.
