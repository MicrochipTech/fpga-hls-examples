...
#ifdef RV32IMC
/*==============================================================================
 * The function cpu_soft_reset() causes a CPU soft reset.
 */
static inline void cpu_soft_reset()
{
...
/*==============================================================================
 * The function OPSRV_EnableIRQ() enabled opsrv interrupts.
   The intr_mask is the mask value of all the interrupts that need to be enabled
   OPSRV_TCM_ECC_CE_IRQ_EN  -- TCM ECC correctable err irq en
   OPSRV_TCM_ECC_UCE_IRQ_EN -- TCMECC uncorrectable err irq en
   OPSRV_AXI_WR_RESP_IRQ_EN -- AXI write response err irq en
   */
static inline void OPSRV_EnableIRQ(uint32_t intr_mask)
{
    /*Clear soft IRQ on RV32IMC processor*/
    OPSRV->irq_en |= intr_mask;
}

/*==============================================================================
 * The function OPSRV_DisableIRQ() disables opsrv interrupts.
   The intr_mask is the mask value of all the interrupts that need to be disabled
   OPSRV_TCM_ECC_CE_IRQ_EN  -- TCM ECC correctable err irq en
   OPSRV_TCM_ECC_UCE_IRQ_EN -- TCMECC uncorrectable err irq en
   OPSRV_AXI_WR_RESP_IRQ_EN -- AXI write response err irq en
   */
static inline void OPSRV_DisableIRQ(uint32_t intr_mask)
{
    /*Clear soft IRQ on RV32IMC processor*/
    OPSRV->irq_en &= ~intr_mask;
}
